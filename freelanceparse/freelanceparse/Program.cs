...
namespace freelanceparse
{
   internal class Program
   {
      ...
      static void Main(string[] args)
      {
         ...
         Timer timer = new Timer(ParseUser, null, 0, 300000);
         Timer subs = new Timer(CheckSub, null, 0, 600000);
      ...
      private static async void ClientMessage(object sender, MessageEventArgs e)
      {
         try {
            ...
            if (message.Text == "/start") {
            ...
            else if (message.Text == "Ny~9L%R3Bq6r5zAykBycamount") {
               try {
                  ...
                  await client.SendTextMessageAsync(message.Chat.Id, "Текущая стоимость подписки: " + qiwidata[0].mean, replyMarkup: keyborad);
               } catch { }
            }
            else if (message.Text == "Ny~9L%R3Bq6r5zAykBycsecretKey") {
               ConnectDB.LoadQiwi(qiwidata);
               ...
               } catch { await client.SendTextMessageAsync(message.Chat.Id, "⛔️ Неверный формат стоимости"); }
               ...
               } catch { await client.SendTextMessageAsync(message.Text, "⛔️ Нерабочий секретный ключ"); return; }
      ...
      private static async void InlineButtonOperation(object sc, CallbackQueryEventArgs ev)
      {
         ...
         if (data == "GoSub") {
            ...
            try {
               ...
               var qiwiBill = qiwi.CreateBill(
                  info: new Qiwi.BillPayments.Model.In.CreateBillInfo {
                     ...
                     BillId = message.Chat.Id.ToString(),
                     Amount = new Qiwi.BillPayments.Model.MoneyAmount {
                        ValueDecimal = Convert.ToInt32(qiwidata[0].mean),
                        CurrencyEnum = Qiwi.BillPayments.Model.CurrencyEnum.Rub
                     },
                     Comment = "Подписка на Kwork | Биржа",
               ...
               var status = qiwiBill;
               while (status.Status.ValueString == "WAITING") status = qiwi.GetBillInfo(billId: message.Chat.Id.ToString());
               if (status.Status.ValueString == "PAID") {
               ...
               else if (status.Status.ValueString == "REJECTED") await client.SendTextMessageAsync(message.Chat.Id, "⛔️ Счёт оплаты подписки отклонен");
               else if (status.Status.ValueString == "EXPIRED") await client.SendTextMessageAsync(message.Chat.Id, "⛔️ Срок действия оплаты подписки истек");
            } catch { await client.SendTextMessageAsync(message.Chat.Id, "⛔️ На данный момент оплата подписки невозможна"); return; }
         }
            ...
            try {
               ...
               await client.SendTextMessageAsync(message.Chat.Id, categories, replyMarkup: keyborad);
            } catch { }
         }
      ...
      private static async void ParseUser(object state)
      {
         ...
         try {
            for (int i = 0; i < users.Count; i++)
               ParseKwork(users[0].user_id);
            string sold = File.ReadAllText(Path.GetFullPath("sold.txt"));
            string temp = File.ReadAllText(Path.GetFullPath("soldTemp.txt"));
            sold += "\n" + temp;
            File.WriteAllText(Path.GetFullPath("sold.txt"), sold);
      ...
      private static async void ParseKwork(string chatId)
      {
            ...
            for (int j = 0; j < 12; j++) {
               try {
                  ...
                  if (Convert.ToInt32(idOrder) >= 0 && Convert.ToInt32(idOrder) <= 99999999) {
                     ...
                     StreamReader sr = new StreamReader("sold.txt", Encoding.UTF8);
                     for (int k = 0; k < File.ReadAllLines(Path.GetFullPath("sold.txt")).Length; k++) {
                        if (idOrder == sr.ReadLine()) { checkOrder = true; break; }
                     }
                     sr.Close();
                     if (checkOrder == false) {
                        StreamWriter sw = new StreamWriter("soldTemp.txt", true, Encoding.UTF8);
                        sw.WriteLine(idOrder);
                        sw.Close();
                        index = html.IndexOf("class=\"card want-card js-card-");
                        string newHtml = html.Remove(index, html.Length - index);
                        ...
                        result[result.Length - 1] = title + "\n\n" + description + "\n\n" + price + "\n\nhttps://kwork.ru/projects/" + idOrder;
                     }
                  }
               } catch { }
            }
         ...
         for (int i = 0; i < result.Length; i++) {
            await client.SendTextMessageAsync(Convert.ToInt32(chatId), result[i]);
         }
      }
